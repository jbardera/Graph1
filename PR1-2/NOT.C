...
VideoBufferSeg=0x0a000;  /* adre‡a inici VGA */
    ...
    asm mov bx,ax         /* carreguem el despla‡ament del pixel a bx */
                          ...
                          /* PixelAddr */
    asm not cl;           /* neguem el valor (color) que hi ha a CL */
...
union REGS ent,sal;
    ...
    {                                     /* del quadrat, ‚s m‚s rapid si es fa */
        ...
        SetPixel(i,j,color1);             /* per• no hi hauria codi C :) */
    }
    for(i=x02;i<=x12;i++)                 /* aquest segon bucle ‚s per si */
    {                                     /* s'entra un segon quadrat, encara */
        for(j=y02;j<=y12;j++)             /* que a l'enunciat de la pr…ctica */
        SetPixel(i,j,color2);             /* no diu res */
    }
